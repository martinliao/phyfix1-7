...
class Regist_contractors extends MY_Controller
{
    ...
    public function index()
    {
        ...
        if (in_array("9", $this->flags->user['group_id'])){
        ...
        foreach ($this->data['list'] as & $row) {
            $row['link_regist'] = base_url("management/regist_contractors/regist/{$row['seq_no']}");
        }
        $this->load->library('pagination');
        $config['base_url'] = base_url("management/regist_contractors?". $this->getQueryString(array(), array('page')));
        $config['total_rows'] = $total;
        $config['per_page'] = $rows;
        $this->pagination->initialize($config);

        $this->data['link_refresh'] = base_url("management/regist_contractors/");

        $this->layout->view('management/regist_contractors/list', $this->data);
    }

    public function regist($seq_no=NULL)
    {
        $this->data['class'] = $this->regist_contractors_model->get($seq_no);
        if(!isset($this->data['class'])){
            $this->setAlert(3, '操作錯誤');
            redirect(base_url('management/regist_contractors/'));
        }
        $conditions = array(
        ...
        if($post = $this->input->post()){
            ...
            redirect(base_url("management/regist_contractors/regist/{$seq_no}"));
        }

        $this->data['import'] = base_url("management/regist_contractors/regist_import/{$seq_no}");
        $this->data['link_cancel'] = base_url("management/regist_contractors/?{$_SERVER['QUERY_STRING']}");
        $this->layout->view('management/regist_contractors/regist', $this->data);
    }
    ...
    public function regist_import($seq_no=NULL)
    {
            ...
            if ($this->_isVerify('add') == TRUE) {
                ...
                if (isset($_FILES['upload']) && $_FILES['upload']['tmp_name'] != '') {
                    if (!fileExtensionCheck($_FILES['upload']['name'], ['csv'])){
                        ...
                        redirect(base_url("management/regist_contractors/regist_import/".$seq_no));
                    }
                    // jd($_FILES);
        ...
        $this->data['link_cancel'] = base_url("management/regist_contractors/regist/{$seq_no}");
        $this->layout->view('management/regist_contractors/upload_csv', $this->data);
    }
                ...
                case 'check_person':
                    ...
                    if($error === TRUE){
                        ...
                        if($person){
                            ...
                            $person['phy_url'] = base_url("management/regist_contractors/phydisabled/{$person['id']}");
                            $result['status'] = TRUE;
